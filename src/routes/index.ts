...
router.get("/api/poems/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (!poem) {
            return res.status(404).json({message: "No poem found"})
        }
...
router.post("/api/poem", async (req: Request, res: Response) => {
    try {
        ...
        if (existingPoem) {
            return res.status(403).json({message: "Poem already existed"})
        }
        ...
        return res.status(201).json({message: "Poem saved successfully"})
